...
public class SVNUtills {
	...
	public String authenticateUser(String username, String password) {
		...
		try {
			SVNURL url = SVNURL.parseURIEncoded(
					"https://nucnocorpsvn.nucleussoftware.com:18080/svn/neo_implementations/hdfc_neo/branches/dev_hdfc_neo/hdfc_neo_3.0/custom_development/neo_intg/source_code/dbscript/Loan_Movement_Patch");
	...
	public static void svn_actions(Patch patchDetail, String username, String password) throws SVNException {
	...
	private static void commitExample(String username, String password) throws SVNException {
		 ...
		 */
		/*
		 * String userName = "dipaliya.jain"; String userPassword = "mar@2020";
		 */
		/*
		 * deepali Sample file contents.
		 */
		/*
		 ...
		 * "D:/SVN_CHECKOUT/dev_hdfc_neo/hdfc_neo_3.0/custom_development/neo_intg/source_code/dbscript/Loan_Movement_Patch/test4/file2.doc"
		 * );
		 * 
		 ...
		 * 
		 * while ((nRead = ips.read(data, 0, data.length)) != -1) { buffer.write(data,
		 * 0, nRead); }
		 * 
		 * buffer.flush();
		 * 
		 ...
		 * 
		 * 
		 * } catch (Exception e) { e.printStackTrace(); } deepali / // byte[] newContent
		 * =
		 * "\n261 \t16/08/19\tAneesha\tNEOSFR-1000\tP-1605-HDFC-PDF_NEOSFR-1207_16.08.2019_295\tY\t17/08/19\tY\t17/08/19"
		 * .getBytes();
		 * 
		 ...
		 * mac.length bytes) System.arraycopy(newContent, 0, finalContent,
		 ...
		 * 
		 * fileData = fileData.replace("kkkkkkkkkkkkkkkkkkkk",
		 * "Not Applicable as we are sharing this release for onsite SIT.");
		 * 
		 * System.out.println(fileData);
		 * 
		 * newContent = fileData.getBytes();
		 * 
		 * /* Create an instance of SVNRepository class. This class is the main entry
		 * point for all "low-level" Subversion operations supported by Subversion
		 * protocol.
		 * 
		 ...
		 * 
		 * Default authentication manager first attempts to use provided user name and
		 * password and then falls back to the credentials stored in the default
		 * Subversion credentials storage that is located in Subversion configuration
		 * area. If you'd like to use provided user name and password only you may use
		 * BasicAuthenticationManager class instead of default authentication manager:
		 * 
		 ...
		 * 
		 * "" (empty string) is path relative to that URL, -1 is value that may be used
		 * to specify HEAD (latest) revision.
		 */
		/*
		 * repository.testConnection(); } catch(SVNException e){
		...
		/*
		 * Checks up if the current path really corresponds to a directory. If it
		 * doesn't, the program exits. SVNNodeKind is that one who says what is located
		 * at a path in a revision.
		 */
		...
		/*
		 * deepali editor = repository.getCommitEditor("file contents changed", null);
		 * 
		 * commitInfo = modifyFile(editor, "test4", "test4/file2.doc", content,
		 * finalContent);
		 * 
		...
		/*
		 * String absoluteSrcPath = repository.getRepositoryPath("test"); long
		 ...
		 * 
		 * editor = repository.getCommitEditor("directory deleted", null); commitInfo =
		 * deleteDir(editor, "test"); System.out.println("The directory was deleted: " +
		 * commitInfo);
		 * 
		 * 
		 * And directory "test2".
		 ...
		 * System.out.println("Repository latest revision (after committing): " +
		 * latestRevision);
		 */
	}
	...
	private static SVNCommitInfo addDir(ISVNEditor editor, String dirPath, String filePath, byte[] data)
		 ...
		 * modifications will be applied to this directory until a next entry (located
		 * inside the root) is opened/added.
		 * 
		...
		editor.openRoot(-1);
		/*
		 * Adds a new directory (in this case - to the root directory for which the
		 * SVNRepository was created). Since this moment all changes will be applied to
		 * this new directory.
		 * 
		 * dirPath is relative to the root directory.
		 * 
		 * copyFromPath (the 2nd parameter) is set to null and copyFromRevision (the
		 * 3rd) parameter is set to -1 since the directory is not added with history (is
		 * not copied, in other words).
		 */
		editor.addDir(dirPath, null, -1);
		/*
		 * Adds a new file to the just added directory. The file path is also defined as
		 * relative to the root directory.
		 *
		 * copyFromPath (the 2nd parameter) is set to null and copyFromRevision (the 3rd
		 * parameter) is set to -1 since the file is not added with history.
		 */
		editor.addFile(filePath, null, -1);
		/*
		 * The next steps are directed to applying delta to the file (that is the full
		 * contents of the file in this case).
		 */
		 ...
		 * 
		 * SVNDeltaGenerator will call editor.textDeltaChunk(...) method for each
		 * generated "diff window" and then editor.textDeltaEnd(...) in the end of delta
		 * transmission. Number of diff windows depends on the file size.
		 * 
		 */
		 ...
		 * information previously described with the editor's methods is sent to the
		 * server for committing. As a result the server sends the new commit
		 * information.
		 */
	...
	private static SVNCommitInfo modifyFile(ISVNEditor editor, String dirPath, String filePath, byte[] oldData,
		 ...
		 * 
		 * Note that you may use only 'target' data to generate delta when there is no
		 * access to the 'base' (previous) version of the file. However, here we've got
		 * 'base' data, what in case of larger files results in smaller network
		 * overhead.
		 * 
		 * SVNDeltaGenerator will call editor.textDeltaChunk(...) method for each
		 * generated "diff window" and then editor.textDeltaEnd(...) in the end of delta
		...
		// ByteArrayInputStream(oldData), 0, new ByteArrayInputStream(newData), editor,
		// true);
	...
	private static SVNCommitInfo copyDir(ISVNEditor editor, String srcDirPath, String dstDirPath, long revision)
		 ...
		 * 
		 * dstDirPath - the destination directory path where the source will be copied
		 * to (relative to the root directory).
		 * 
